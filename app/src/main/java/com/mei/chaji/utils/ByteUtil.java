...
//    }
     ...
     * 转二进制补码
     */
    public static String toBinarStr(String hexstr) {
        int int_bin = (Integer.valueOf(hexstr, 16));
        String tobiner = Integer.toBinaryString(int_bin);
        if (tobiner.length() < 8) {
            while (tobiner.length() < 8) {
                StringBuffer sb = new StringBuffer();
                sb.append("0").append(tobiner);
                //倒序处理
                tobiner = sb.toString();
            }
        }
