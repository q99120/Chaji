package com.mei.chaji.ui.other;
...
import com.mei.chaji.ui.main.adapter.BannerViewAdapter;
...
public class Banners extends RelativeLayout {
    ...
    VideoUtil videoUtil;
    int i_flag = 0;
    List<VideoUtil> videolist = new ArrayList<>();
    ...
    public Banners(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public Banners(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public Banners(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public void setDataList(List<VideoUtil> dataList) {
        ...
        if (dataList.size() > 1) {
            ...
            for (int i = 0; i < dataList.size() + 2; i++) {
                String url;
                if (i == 0) {
                    url = dataList.get(dataList.size() - 1).getAd_url();
                } else if (i == dataList.size() + 1) {
                    url = dataList.get(0).getAd_url();
                } else {
                    url = dataList.get(i - 1).getAd_url();
                }
            ...
            autoCurrIndex = 0;
            String url = dataList.get(0).getAd_url();
            if (MimeTypeMap.getFileExtensionFromUrl(url).equals("mp4")) {
                MVideoView videoView = new MVideoView(getContext());
                videoView.setLayoutParams(lp);
                videoView.setVideoURI(Uri.parse(url));
    ...
    private Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            //只有一个条件，所以选择使用if而不是switch
            if (msg.what == UPTATE_VIEWPAGER) {
                if (i_flag < videolist.size() - 1) {
                    i_flag += 1;
                } else {
                    i_flag = 0;
                }
                if (autoCurrIndex == videolist.size()) {
                    i_flag = 0;
                }
                Log.e("===========", "handleMessage: " + videolist.get(i_flag).getAd_url());
                imgDelyed = Integer.parseInt(videolist.get(i_flag).getAd_time()) * 1000;
